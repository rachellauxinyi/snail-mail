  ...
  for (const pending of pendingLetters) {
    try {
      ...
      if (response.ok) {
        ...
        processed++;
        console.log(`Delivered letter ${letter.letterId} to ${recipientEmail}`);
      } else {
        errors++;
...
app.post("/make-server-4ba6ddf6/send-email", async (c) => {
  try {
    ...
    console.log(`Letter stored: ${letterId}, delivery: ${deliveryDate.toISOString()}`);

    const edgeRt = (globalThis as Record<string, { waitUntil(p: Promise<unknown>): void }>)["EdgeRuntime"];
    edgeRt?.waitUntil?.(
      (async () => {
